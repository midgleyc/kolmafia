...
public class FightRequest extends GenericRequest {
  ...
  private static boolean extractVerse(
    ...
    for (Node child : node.childNodes()) {
      if (child instanceof TextNode tn) {
        buffer.append(tn.getWholeText());
      } else if (child instanceof Element) {
        hasTag |= FightRequest.extractVerse((Element) child, buffer, tag);
      }
    }
  ...
  private static Element parseFamBattleHTML(String text, boolean logIt) {
    ...
    Document node = FightRequest.cleanFightHTML(text);
    if (node == null) {
      if (logIt) {
    ...
    while (it.hasNext() && !done) {
      ...
      if (child instanceof Element tnode) {
        ...
        if (name.equals("table")) {
          ...
          if (tnode.text().equals("Your Team")) {
            done = true;
          }
        }
      }
  ...
  */
  ...
  public static final void parsePokefam(final Element node, boolean myFamiliar, boolean moveSpans) {
    ...
    Elements row1Tags = rows.first().children();
    int td = 1;
    ...
    if (row2Tags.size() > 0) {
      String famtype = row2Tags.get(0).text();
  ...
  private static Document cleanFightHTML(final String text) {
  ...
  private static String getTextNodeText(Element node) {
    StringBuilder text = new StringBuilder();
    for (Node item : node.childNodes()) {
      if (item instanceof TextNode tn) {
        text.append(tn.text());
      }
    }
  ...
  private static void processNode(final Element node, final TagStatus status) {
    ...
    if (name.equals("script")) {
      Matcher m = CLEESH_PATTERN.matcher(node.text());
      if (!m.find()) {
        return;
      }
    ...
    if (name.equals("table")) {
      ...
      if (cl != null && cl.equals("item") && rel != null) {
        ...
        if (node.text().startsWith("Item unequipped")) {
      ...
      var tables = node.children().select("table");
    ...
    if (name.equals("hr")) {
      ...
      String str = FightRequest.getTextNodeText(node);
    ...
    if (inode != null) {
      ...
      if (alt.startsWith("Enemy's")) {
        ...
        for (int i = 0; i < cells.size(); i++) {
          ...
          i++;
          cell = cells.get(i);
          int value = StringUtilities.parseInt(cell.text());
          switch (stat) {
  ...
  private static void processFamiliarAction(Element node, Element inode, TagStatus status) {
    ...
    var tables = node.select("table").stream().filter(x -> x != node).toList();
    for (Element table : tables) {
      table.remove();
    }
